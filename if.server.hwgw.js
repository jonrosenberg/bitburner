...
export function numCycleForGrowthCorrected(server, targetMoney, startMoney, hackMult, cores = 1, ns=null) {
  if (startMoney < 0) { startMoney = 0; } // servers "can't" have less than 0 dollars on them
	...
	const adjGrowthRate = (1 + (ServerBaseGrowthRate - 1) / server.hackDifficulty);
  const exponentialBase = Math.min(adjGrowthRate, ServerMaxGrowthRate); // cap growth rate
  
	// total of all grow thread multipliers
	const serverGrowthPercentage = server.serverGrowth / 100.0;
  const coreMultiplier = 1 + ((cores - 1) / 16);
  const threadMultiplier = serverGrowthPercentage * hackMult.growth * coreMultiplier * BitNodeMultipliers.ServerGrowthRate;
...
export default class HWGWBaseServer extends HackableBaseServer {
    ...
    get perfect_batch() {
        ...
        const growThreads = numCycleForGrowthCorrected(this.ns.getServer(this.id), this.money.max, this.money.max * (1 - this.calculate_hack_percent()) , this.ns.getHackingMultipliers(), home.cores, this.ns);
        return {
            hk: hackThreads,
            gr: growThreads,
