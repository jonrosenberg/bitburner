...
import { findEffectiveLibertiesOfNewMove } from "/go/boardAnalysis/boardAnalysis"
import { floor } from "go/boardState/boardState"
  ...
  for (let x = 0; x < boardSize; x++) {
    for (let y = 0; y < boardSize; y++) {
      ...
      const matchedPattern = patterns.find(pattern =>
        checkMatch(neighborhood, pattern, player)
      )

      if (
        matchedPattern &&
        availableSpaces.find(
          availablePoint => availablePoint.x === x && availablePoint.y === y
        ) &&
        (!smart ||
          findEffectiveLibertiesOfNewMove(board, x, y, player).length > 1)
      ) {
...
function checkMatch(neighborhood, pattern, player) {
  ...
  return patternArr.every((str, index) =>
    matches(str, neighborhoodArray[index], player)
  )
}
...
function expandAllThreeByThreePatterns() {
  ...
  const mirroredPatterns = [
    ...rotatedPatterns,
    ...rotatedPatterns.map(verticalMirror)
  ]
