import { GoValidity, GoOpponent, GoColor } from "/go/enums"
// import { Go } from "../Go"
import {
...
export function evaluateIfMoveIsValid(
  ...
  const possibleRepeat = boardState.previousBoards.find(
    board => getColorOnSimpleBoard(board, x, y) === player
  )
  ...
  if (shortcut) {
    ...
    const hasLiberty =
      liberties.north || liberties.east || liberties.south || liberties.west
    ...
    const neighborChainLibertyCount = findMaxLibertyCountOfAdjacentChains(
      boardState,
      x,
      y,
      player
  ...
  const evaluationBoard = evaluateMoveResult(
    boardState.board,
    x,
    y,
    player,
  ...
  if (possibleRepeat && boardState.previousBoards.length) {
    ...
    if (
      boardState.previousBoards.find(board =>
        areSimpleBoardsIdentical(simpleEvalBoard, board)
      )
    ) {
 ...
 */
export function evaluateMoveResult(board, x, y, player, resetChains = false) {
...
export function getControlledSpace(board) {
  const chains = getAllChains(board)
  const length = board[0].length
  const whiteControlledEmptyNodes = getAllPotentialEyes(
    board,
    chains,
    GoColor.white,
    length * 2
    ...
    .map(eye => eye.chain)
    .flat()
  const blackControlledEmptyNodes = getAllPotentialEyes(
    board,
    chains,
    GoColor.black,
    length * 2
  ...
  const ownedPointGrid = Array.from({ length }, () =>
    Array.from({ length }, () => GoColor.empty)
  )
 ...
 */
export function findEffectiveLibertiesOfNewMove(board, x, y, player) {
  ...
  const allyNeighbors = neighborPoints.filter(
    neighbor => neighbor.color === player
  )
  const allyNeighborChainLiberties = allyNeighbors
    .map(neighbor => {
      const chain = friendlyChains.find(
        chain => chain[0].chain === neighbor.chain
      )
  ...
  const directLiberties = neighborPoints.filter(
    neighbor => neighbor.color === GoColor.empty
  )
  ...
  return allLiberties
    .filter(
      (liberty, index) =>
        allLiberties.findIndex(
          neighbor => liberty.x === neighbor.x && liberty.y === neighbor.y
        ) === index
    )
 ...
 */
export function findMaxLibertyCountOfAdjacentChains(boardState, x, y, player) {
  const neighbors = findAdjacentLibertiesAndAlliesForPoint(
    boardState.board,
    x,
    y,
    player
  ...
  return friendlyNeighbors.reduce(
    (max, neighbor) => Math.max(max, neighbor?.liberties?.length ?? 0),
    0
  )
}
 ...
 */
export function findMinLibertyCountOfAdjacentChains(board, x, y, player) {
  const chain = findEnemyNeighborChainWithFewestLiberties(board, x, y, player)
  return chain?.[0]?.liberties?.length ?? 99
}

export function findEnemyNeighborChainWithFewestLiberties(board, x, y, player) {
  const chains = getAllChains(board)
  const neighbors = findAdjacentLibertiesAndAlliesForPoint(board, x, y, player)
  const friendlyNeighbors = [
    neighbors.north,
  ...
  const chainId = friendlyNeighbors.find(
    neighbor => neighbor?.liberties?.length === minimumLiberties
  )?.chain
 ...
 */
export function getAllValidMoves(boardState, player) {
  return getEmptySpaces(boardState.board).filter(
    point =>
      evaluateIfMoveIsValid(boardState, point.x, point.y, player) ===
      GoValidity.valid
  )
}
 ...
 */
export function getAllEyesByChainId(board, player) {
 ...
 */
export function getAllEyes(board, player, eyesObject) {
 ...
 */
export function getAllPotentialEyes(board, allChains, player, _maxSize) {
  const nodeCount = board.map(row => row.filter(p => p)).flat().length
  const maxSize = _maxSize ?? Math.min(nodeCount * 0.4, 11)
  const emptyPointChains = allChains.filter(
    chain => chain[0].color === GoColor.empty
  )
  const eyeCandidates = []
  ...
  emptyPointChains
    ...
    .forEach(chain => {
      ...
      if (
        (hasWhitePieceNeighbor &&
          !hasBlackPieceNeighbor &&
...
function findNeighboringChainsThatFullyEncircleEmptySpace(
  ...
  return neighborChainList.filter((neighborChain, index) => {
    ...
    const couldWrapSouth =
      neighborSpread.south < candidateSpread.south ||
      (candidateSpread.south === 0 && neighborSpread.south === 0)
    const couldWrapWest =
      neighborSpread.west < candidateSpread.west ||
    ...
    const otherChainNeighborPoints = removePointAtIndex(
      neighborChainList,
      index
    )
      .flat()
 ...
 */
export function getAllNeighboringChains(board, chain, allChains) {
  ...
  const neighboringChains = playerNeighbors.reduce(
    (neighborChains, neighbor) =>
      neighborChains.add(
        allChains.find(chain => chain[0].chain === neighbor.chain) || []
      ),
    new Set()
  )
 ...
 */
export function getPlayerNeighbors(board, chain) {
  return getAllNeighbors(board, chain).filter(
    neighbor => neighbor && neighbor.color !== GoColor.empty
  )
}
 ...
 */
export function getAllNeighbors(board, chain) {
 ...
 */
export function isPointInChain(point, chain) {
  return !!chain.find(
    chainPoint => chainPoint.x === point.x && chainPoint.y === point.y
  )
}
 ...
 */
export function getAllChains(board) {
  const chains = {}
 ...
 */
export function findAllCapturedChains(chainList, playerWhoMoved) {
  const opposingPlayer =
    playerWhoMoved === GoColor.white ? GoColor.black : GoColor.white
  const enemyChainsToCapture = findCapturedChainOfColor(
    chainList,
    opposingPlayer
  )
  ...
  const friendlyChainsToCapture = findCapturedChainOfColor(
    chainList,
    playerWhoMoved
  )
...
function findCapturedChainOfColor(chainList, playerColor) {
  return chainList.filter(
    chain =>
      chain?.[0].color === playerColor && chain?.[0].liberties?.length === 0
  )
}
 ...
 */
export function findLibertiesForChain(board, chain) {
  return getAllNeighbors(board, chain).filter(
    neighbor => neighbor && neighbor.color === GoColor.empty
  )
}
 ...
 */
export function findChainLibertiesForPoint(board, x, y) {
 ...
 */
export function findAdjacentLibertiesForPoint(board, x, y) {
  const neighbors = findNeighbors(board, x, y)

  const hasNorthLiberty =
    neighbors.north && neighbors.north.color === GoColor.empty
  const hasEastLiberty =
    neighbors.east && neighbors.east.color === GoColor.empty
  const hasSouthLiberty =
    neighbors.south && neighbors.south.color === GoColor.empty
  const hasWestLiberty =
    neighbors.west && neighbors.west.color === GoColor.empty
 ...
 */
export function findAdjacentLibertiesAndAlliesForPoint(board, x, y, _player) {
  const currentPoint = board[x]?.[y]
  const player =
    _player ||
    (!currentPoint || currentPoint.color === GoColor.empty
      ? undefined
      : currentPoint.color)
  ...
  return {
    north:
      adjacentLiberties.north || neighbors.north?.color === player
        ? neighbors.north
        : null,
    east:
      adjacentLiberties.east || neighbors.east?.color === player
        ? neighbors.east
        : null,
    south:
      adjacentLiberties.south || neighbors.south?.color === player
        ? neighbors.south
        : null,
    west:
      adjacentLiberties.west || neighbors.west?.color === player
        ? neighbors.west
        : null
  }
}
 ...
 *
 */
export function simpleBoardFromBoard(board) {
...
export function boardFromSimpleBoard(simpleBoard) {
...
export function boardStateFromSimpleBoard(
  ...
  const newBoardState = getNewBoardState(
    simpleBoard[0].length,
    ai,
    false,
    boardFromSimpleBoard(simpleBoard)
  )
...
export function blankPointState(color, x, y) {
  return {
    color: color,
    y,
    x,
...
export function areSimpleBoardsIdentical(simpleBoard1, simpleBoard2) {
  return simpleBoard1.every((column, x) => column === simpleBoard2[x])
}

export function getColorOnSimpleBoard(simpleBoard, x, y) {
...
export function getPreviousMove() {
  ...
  //       point &&
  //       priorBoard &&
  //       getColorOnSimpleBoard(priorBoard, point.x, point.y)
