...
import { contains, isNotNull } from "/go/boardState/boardState"
 ...
 */
export function findDisputedTerritory(boardState, player, excludeFriendlyEyes) {
  ...
  const emptySpacesToAnalyze = getAllPotentialEyes(
    boardState.board,
    chains,
    opponent
  )
  const nodesInsideEyeSpacesToAnalyze = emptySpacesToAnalyze
    .map(space => space.chain)
    .flat()

  const playableNodesInsideOfEnemySpace = emptySpacesToAnalyze.reduce(
    (playableNodes, space) => {
      ...
      const attackableLiberties = space.neighbors
        .map(neighborChain => {
          ...
          const neighborChains = getAllNeighboringChains(
            boardState.board,
            neighborChain,
            chains
          )
  ...
  return validMoves.filter(
    move =>
      !contains(nodesInsideEyeSpacesToAnalyze, move) ||
      contains(playableNodesInsideOfEnemySpace, move)
  )
}
 ...
 */
export function findClaimedTerritory(board) {
  const whiteClaimedTerritory = getAllEyes(board, GoColor.white).filter(
    eyesForChainN => eyesForChainN.length >= 2
  )
  const blackClaimedTerritory = getAllEyes(board, GoColor.black).filter(
    eyesForChainN => eyesForChainN.length >= 2
  )
