// import { Board, BoardState, EyeMove, Move, MoveOptions, Play, PointState } from "../Types";
// import { Player } from "@player"
import { AugmentationName, GoOpponent, GoColor, GoPlayType } from "/go/enums"
import { opponentDetails } from "/go/Constants"
import {
  findNeighbors,
  floor,
  isDefined,
...
import { findDisputedTerritory } from "/go/boardAnalysis/controlledTerritory"
import { findAnyMatchedPatterns } from "/go/boardAnalysis/patternMatching"
// import { Go, GoEvents } from "../Go"
//import { WHRNG } from "../../Casino/RNG"
function rng_random() { return Math.random(); }
...
export async function main(ns) {
  ...
  const op = 'Netburners'
  const bsize = 5
  simpleNewBoard = ns.go.resetBoardState(op,bsize)
  ns.tail();
  ns.print("start");
  //let newBoardState = getNewBoardState(ns,bsize,op,false,simpleNewBoard);
  // const play = makeAIMove(ns,newBoardState)
  // ns.print(play);
}
 ...
 */
export function makeAIMove(ns, boardState) {
...
export async function getMove(boardState, player, opponent, rngOverride) {
  ...
  const smart = isSmart(opponent, rng_random())
  const moves = getMoveOptions(boardState, player, rng_random(), smart)

  const priorityMove = await getFactionMove(moves, opponent, rng_random())
  if (priorityMove) {
    return {
  ...
  const chosenMove = moveOptions[floor(rng_random() * moveOptions.length)]
