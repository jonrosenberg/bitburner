import { GoOpponent, GoColor, GoValidity } from "/go/enums"
import { bitverseBoardShape } from "/go/Constants"
import { getExpansionMoveArray } from "/go/boardAnalysis/goAI"
import {
 ...
 * @param {NS} ns
*/
export function getNewBoardState(ns,
  boardSize,
  ...
  //   boardToCopy = resetCoordinates(
  //     rotate90Degrees(boardFromSimpleBoard(bitverseBoardShape))
  //   )
  //   boardSize = 19
  //   applyObstacles = false
  // }
 ...
 */
export function getHandicap(boardSize, opponent) {
  // Illuminati and WD get a few starting routers
  if (
    opponent === GoOpponent.Illuminati ||
 ...
 */
export function makeMove(boardState, x, y, player) {
 ...
 */
export function passTurn(boardState, player, allowEndGame = true) {
  if (
 ...
 */
export function applyHandicap(board, handicap) {
 ...
 */
export function updateChains(board, resetChains = true) {
 ...
 * Modifies the board in place.
 */
export function updateCaptures(board, playerWhoMoved, resetChains = true) {
 ...
 */
export function findAdjacentPointsInChain(board, x, y) {
  const point = board[x][y]
  if (!point) {
    return []
  }
 ...
 */
export function getEmptySpaces(board) {
  const emptySpaces = []
 ...
 */
export function getStateCopy(initialState) {
...
export function contains(arr, point) {
  return !!arr.find(p => p && p.x === point.x && p.y === point.y)
}

export function findNeighbors(board, x, y) {
  return {
    north: board[x]?.[y + 1],
    east: board[x + 1]?.[y],
    south: board[x]?.[y - 1],
...
export function getArrayFromNeighbor(neighborObject) {
  return [
    neighborObject.north,
    neighborObject.east,
    neighborObject.south,
    neighborObject.west
  ]
    .filter(isNotNull)
    .filter(isDefined)
